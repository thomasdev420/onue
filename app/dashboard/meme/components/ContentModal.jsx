...
export default function ContentModal({
  ...
  const renderContent = () => {
    ...
    if (contentType === 'stock') {
      imagesToShow = backgroundImages;
    } else if (contentType === 'user') {
      imagesToShow = userImages;
    }
    return (
      <div className="grid grid-cols-4 gap-2 p-2">
        {contentType === 'user' && (!userImages || userImages.length === 0) && (
          <div className="text-gray-400 col-span-4 text-center py-4">No uploads yet</div>
        )}
        {imagesToShow && imagesToShow.length > 0 && imagesToShow.map((image, idx) => (
          <div
            key={image.id || idx}
          ...
          >
            <Image
              src={image.image_url || image.url || image.src}
              alt={image.title || image.alt || 'User image'}
              fill
              sizes="200px"
