...
export default function HookDemoEditor() {
  ...
  const [mode, setMode] = useState('hook-demo');
  ...
  // Use persistence hook for hook-demo - INDEPENDENT from slides
  const defaultHookDemo = [{ id: `${Date.now()}-${Math.floor(Math.random() * 1000000)}`, image: null, texts: [], ratio: '16:9' }];
  const { 
    data: hookDemo, 
    updateData: setHookDemo, 
    resetData: resetHookDemo,
    saveStatus, 
    isLoading: isLoadingHookDemo 
  } = usePersistence('hook-demo', defaultHookDemo);

  const { data: userImages } = usePersistence('userImages', []);

  // Active hook-demo state
  const [activeHookDemoIndex, setActiveHookDemoIndex] = useState(0);

  // Custom hooks for hook-demo management
  const {
    updateSlide: updateHookDemo,
    addSlide: addHookDemo,
    deleteSlide: deleteHookDemo,
    changeRatio,
    handleSelectImageForSlide: handleSelectImageForHookDemo
  } = useSlideManagement({
    slides: hookDemo,
    setSlides: setHookDemo,
    activeSlideIndex: activeHookDemoIndex,
    setActiveSlideIndex: setActiveHookDemoIndex
  });
  ...
  // Add a guard to prevent rendering with invalid hook-demo data
  if (isLoadingHookDemo || !hookDemo || hookDemo.length === 0) {
    return (
      <div className="p-8">
        <h1 className="text-2xl font-bold text-gray-800 mb-8">Demo Editor</h1>
        <div className="bg-white rounded-lg shadow-sm p-6">
          <p className="text-gray-600">Loading your demo...</p>
        </div>
      </div>
    );
  }
    ...
    <>
      ...
      <div style={{ position: 'fixed', top: 24, right: 160, zIndex: 1100 }}>
        <button
          ...
          style={{
            background: modeColorMap[mode] || '#DC2626',
            border: 'none',
            borderRadius: '9999px',
            boxShadow: `0 2px 8px 0 ${(modeColorMap[mode] || '#DC2626')}22, 0 0 0 1px ${(modeColorMap[mode] || '#DC2626')}11`,
            color: '#fff',
            fontWeight: 500,
            fontSize: 18,
            padding: '12px 32px',
      ...
      <div className="p-8">
        <div className="flex items-center justify-between mb-8">
          <h1 className="text-2xl font-bold text-gray-800">Demo Editor</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-lg shadow-sm p-6">
              <SlideCanvas
                slide={hookDemo[activeHookDemoIndex]}
                onUpdate={(updatedSlide) => updateHookDemo(activeHookDemoIndex, updatedSlide)}
                onImageSelect={handleSelectImageForHookDemo}
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-lg shadow-sm p-6">
              <SlideControls
                slides={hookDemo}
                activeIndex={activeHookDemoIndex}
                onSlideChange={setActiveHookDemoIndex}
                onAddSlide={addHookDemo}
                onDeleteSlide={deleteHookDemo}
                onReset={resetHookDemo}
                mode="hook-demo"
              />
            </div>
          </div>
